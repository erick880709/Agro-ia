...
1. **Busca definiciones existentes.** Revisa si hay contenido en `resources/architecture/definitions/` (RNF/RT de `janus`) y en `resources/design/models/` (RD de `janus`) (rutas relativas a la raíz del proyecto). Si cualquiera de las dos carpetas existe y tiene archivos, léelos **individualmente por ID** (no como un bloque de texto genérico) antes de seguir — pueden traer estándares corporativos, decisiones ya tomadas, o modelos de dominio/datos que no debes reinventar ni contradecir sin justificarlo explícitamente.
   - Si un `RD-###` concreto tiene ambigüedades marcadas en sus "Notas del analista" que afectan una decisión arquitectónica crítica (ej. un lineamiento de identidad visual tan vago que no permite decidir el enfoque de theming, o una entidad de datos mencionada sin campos), no lo asumas ni lo completes por tu cuenta: sugiere al usuario refinarlo primero con `epicureo` (`epicureo RD-00N`) y retómalo cuando vuelva con el IA bajo, en vez de inventar la parte que falta.
...
## Paso 0.6: Revisa el desglose funcional ya resuelto por `specter` (Casos A y C)

Si `specter` ya corrió sobre este proyecto, parte de lo que vas a diseñar en la Sección 9
...
2. Si existen, lee la sección "Recurso de datos involucrado" de cada uno (cuando esté
   presente — `specter` la omite en historias que no tocan datos). `specter` tiene la
   política de no inventar campos que la épica no sustente (`specter/SKILL.md` §Reglas de
...
3. Usa esos campos/tipos/relaciones como punto de partida obligatorio de la Sección 9: no
   rediseñes una entidad que `specter` ya resolvió, y no la contradigas sin justificar
...
5. Si ningún HU tiene esta sección resuelta (no se usó `specter`, o ninguna historia tocaba
...
1. **Aplica el Paso 0.5** para validar/completar la línea base de desarrollo antes de leer las especificaciones en detalle, **el Paso 0.6** para revisar si `specter` ya resolvió parte del modelo de datos, **y el Paso 0.7 si el modo ML/IA está activo**.
...
2. **Aplica el Paso 0.5** para validar/completar la línea base de desarrollo del TO-BE (no confundir con el AS-IS: aquí valida qué de la línea base cambia respecto a lo ya implementado), **y el Paso 0.6** para el modelo de datos objetivo.
...
- En Casos A/C: ¿se revisó `resources/functional/hu/` (Paso 0.6) antes de diseñar la Sección 9? Si `specter` ya había resuelto el "Recurso de datos involucrado" de alguna HU/TT, ¿el Modelo de Datos es consistente con eso, o se documentó explícitamente por qué se apartó?
