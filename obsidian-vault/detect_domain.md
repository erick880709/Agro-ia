---
title: "detect_domain"
tags: [agroia, community, code, rationale]
community_id: 112
node_count: 7
---

# detect_domain

> **7 nodos** | Tipos: code, rationale

## 📄 Archivos principales

- `.github/skills/figma-prd-mockups/references/ui-ux-pro-max-skill-main/cli/assets/scripts/tests/test_core.py` (5 símbolos)
- `.github/skills/figma-prd-mockups/references/ui-ux-pro-max-skill-main/cli/assets/scripts/core.py` (2 símbolos)

## 🔗 Nodos clave

- **detect_domain()** `[code]` _core.py_
- **Auto-detect the most relevant domain from query. Matches are weighted by…** `[rationale]` _core.py_
- **TestDomainDetection** `[code]` _test_core.py_
- **.test_style_keywords_route_to_style()** `[code]` _test_core.py_
- **.test_accessibility_keywords_route_to_ux()** `[code]` _test_core.py_
- **.test_ambiguous_query_returns_runner_up()** `[code]` _test_core.py_
- **.test_empty_query_falls_back_to_style()** `[code]` _test_core.py_

## 🌐 Comunidades conectadas

- [[DesignSystemGenerator|DesignSystemGenerator]] (22 nodos)
- [[scripts⁄core.py|scripts/core.py]] (14 nodos)
- [[assets⁄scripts⁄tests⁄test_core.py|assets/scripts/tests/test_core.py]] (14 nodos)
- [[search|search]] (11 nodos)
- [[cip⁄core.py|cip/core.py]] (9 nodos)
